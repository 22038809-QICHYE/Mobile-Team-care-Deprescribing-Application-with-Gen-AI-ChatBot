    ...
    if "add_resource" in request.form:
        ...
        if file and allowed_file(file.filename):
            ...
            if filename.lower().endswith('.pdf'):
                documents = Ingestion_file().chunk_pdf_text(filename)
                if documents:
                    ChromaManager().add_documents(documents)
                    flash(f"Processed and indexed {len(documents)} documents!")
            elif filename.lower().endswith('.csv'):
                documents = Ingestion_file().chunk_csv_text(filename)
                if documents:
                    ChromaManager().add_documents(documents)
                    flash(f"Processed and indexed {len(documents)} documents!")
            else:
        ...
        if resource:
            ...
            ChromaManager().delete_document_via_metadata(metadata_id=resource)
