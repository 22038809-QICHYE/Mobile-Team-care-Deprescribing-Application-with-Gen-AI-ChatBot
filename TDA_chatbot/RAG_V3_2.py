...
from Re_ranker import CrossEncoderReRanker, BM25ReRanker
from Memory import RedisSemanticCacheManager, ExactMatchRedisCache
...
class RAGSystem:
    def __init__(self):
        ...
        self.re_ranker2 = BM25ReRanker()
        self.augmenter = Augmentation()
        self.cache_manager = RedisSemanticCacheManager()
        self.cache_manager2 = ExactMatchRedisCache() # exact match dont need llm_string

    # Generation engine's query(original) with original query reranking
    ...
    # GPT4 generated Multi query with generated query reranking
    def process_query_v2(self, query: str):
        ...
        try:
            # Step 1: Check cache
            cached_document = self.cache_manager2.lookup(query)
            if cached_document:
                print(f"Cache hit! Cached document:\n{cached_document}")
                return cached_document
            ...
            # Step 5: Update cache
            self.cache_manager2.update(query, augmented_query_data)

            return augmented_query_data
        except Exception as e:
            print(f"\nAn error occurred during testing: {e}")
    # GPT4 generated Multi query with original query reranking
    def process_query_mix(self, query: str):
            ...
            try:
                ...
                print(f"\nAn error occurred during testing: {e}")
    # GPT4 generated Multi query with BM25 reranking 
    def process_query_v3(self, query: str):
        """
        Process an LLM query through Multi query retrieval, Multi query re-ranking, and caching.
        :param query: The input query.
        :return: Augmented query (with context).
        ...
        try:
            ...
            # Step 3: Re-rank the retrieved documents across all queries
            print("\n--- Re-ranking Documents Across Queries ---")
            reranked_documents = self.re_ranker2.rerank_documents(query, retrieved_docs)

            # Display the re-ranked results
            print("\nRe-ranked Documents Across Queries:")
            print(self.re_ranker2.format_results(reranked_documents))

            # Step 4: Augment query with top document
            augmented_query_data = self.augmenter.augment_query_with_document2(query, reranked_documents)
...
def test_v3():
    ...
    # Process the query through the RAG pipeline
    result = rag.process_query_v3(query)
